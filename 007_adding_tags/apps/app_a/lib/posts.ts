...
export function getAllTagsIds(): { params: { id: string } }[] {
  const tagsIds: Set<string> = new Set([]);
  const fileNames = fs.readdirSync(postsDirectory);
  ...
  fileNames.forEach((fileName) => {
    ...
    const postTags: string[] = matterResult.data.tags ?? [];
    postTags.forEach((tag) => {
      tagsIds.add(tag);
    });
  });

  return Array.from(tagsIds).map((tag) => {
    return {
      params: {
...
export function getPostsByTag(tag: string): {
  ...
  const filteredPosts = allPostsData.filter((e) => {
    ...
    return mappedTags.includes(tag.toLocaleLowerCase());
  });
